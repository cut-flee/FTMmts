// usk_ausgangsmodell_v01~model.c
...
newattrib(load* this, load* that)
{
	...
	if (!this->attribute)
		...
		if (this->attribute->am_model.am_A_Str)
			xfree(this->attribute->am_model.am_A_Str);
	}
	if (that != NULL) {
		*(this->attribute) = *(that->attribute);
		if (this->attribute->am_model.am_A_Str) {
			this->attribute->am_model.am_A_Str = strdup(that->attribute->am_model.am_A_Str);
		}
	} else
...
get_ldatt_address(load* this, int32 index, int32* d)
{
	...
	switch(index) {
	case 0:
		data = (void*)&this->attribute->am_model.am_A_Nummer_NS;
		break;
	case 1:
		data = (void*)&this->attribute->am_model.am_A_Str;
		break;
	case 2:
		data = (void*)&this->attribute->am_model.am_A_Next_Station;
		break;
	case 3:
		data = (void*)&this->attribute->am_model.am_A_Zustand;
		break;
	case 4:
		data = (void*)&this->attribute->am_model.am_A_Modulo;
		break;
	}
	return data;
}
...
set_attdata(load* this)
{
	...
	am_model.am_A_Nummer_NS$att->data = (void*)&this->attribute->am_model.am_A_Nummer_NS;
	am_model.am_A_Str$att->data = (void*)&this->attribute->am_model.am_A_Str;
	am_model.am_A_Next_Station$att->data = (void*)&this->attribute->am_model.am_A_Next_Station;
	am_model.am_A_Zustand$att->data = (void*)&this->attribute->am_model.am_A_Zustand;
	am_model.am_A_Modulo$att->data = (void*)&this->attribute->am_model.am_A_Modulo;
}
